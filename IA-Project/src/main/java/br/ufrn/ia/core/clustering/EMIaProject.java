...
public class EMIaProject extends RandomizableDensityBasedClusterer
	...
	public void setOptions(	boolean m_verbose,
			...
			this.m_verbose = m_verbose; 								//default false
			this.m_max_iterations = m_max_iterations; 					//default 100
			this.m_num_clusters = m_num_clusters; 						//default -1
			this.m_minStdDev = m_minStdDev; 							//default 1.0E-6
			...
			for (j = 0; j < m_num_attribs; j++) {
				if (inst.attribute(j).isNominal()) {
					...
					double stdv = (stdD.instance(i).isMissing(j))
							? ((m_maxValues[j] - m_minValues[j]) / (2 * m_num_clusters)) : stdD.instance(i).value(j);
					if (stdv < minStdD) {
		...
		for (j = 0; j < m_num_clusters; j++) {
			for (i = 0; i < m_num_attribs; i++) {
				...
				if (m_theInstances.attribute(i).isNominal()) {
					...
					System.out
							.println("Normal Distribution. Mean = " + Utils.doubleToString(m_modelNormal[j][i][0], 8, 4)
